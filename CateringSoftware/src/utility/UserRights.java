...
public class UserRights extends javax.swing.JInternalFrame {
    ...
    private void setUserValues() {
        try {
            ...
            ResultSet result;
            pst = dataConnection.prepareStatement("SELECT username FROM user_master", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    private void setPrivileges(TreePath path) {
        ...
        String query = "SELECT u.views, u.print, u.edit, u.adds, u.deletes, u.navigate_view "
                + "FROM form_master f, user_rights u WHERE u.user_cd = "+ lb.getUserName(userRights.get(1).toString(), "C") +" AND f.name = '"+ (String) userRights.get(3) +"' AND (f.id = u.form_cd)";
        if(!DeskFrame.forms.isEmpty()){
            query+=" AND f.id IN ("+ DeskFrame.forms +")";
        }
        try {
    ...
    private void applyPrivileges() {
        ...
        String query = "UPDATE user_rights SET views = "+ flag[0] +", edit = "+ flag[2] +", adds = "+ flag[3] +", deletes = "+ flag[4] +", print = "+ flag[1] +", navigate_view = "+ flag[5] +""
            ...
            + " form_cd = (SELECT id FROM form_master WHERE name = '"+ userRights.get(3).toString() +"')";
        try {
            PreparedStatement pst = dataConnection.prepareStatement(query);
            pst.executeUpdate();
    ...
    public static void setUserRightsToPanel(SmallNavigation1 navLoad, String FormID) {
    ...
    private void addUserMenu(TreeNode node) {
        try {
            ...
            String query = "SELECT name FROM form_master WHERE fk_menu_id IN "
                + "(SELECT id FROM menu_master WHERE name = '"+ node.toString() +"')";
            if (!DeskFrame.forms.isEmpty()) {
                query += " AND id IN ("+ DeskFrame.forms +")";
            }
    ...
    private void addFormToUser(TreeNode node) {
        try {
            ...
            ResultSet result;
            DefaultMutableTreeNode parent = (DefaultMutableTreeNode) node;
            pst = dataConnection.prepareStatement("SELECT name FROM menu_master", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
            result = pst.executeQuery();
